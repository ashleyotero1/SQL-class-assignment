...
FROM 
    TPassengers as TP
JOIN 
    TFlightPassengers AS TFP ON TP.intPassengerID = TFP.intFlightID
JOIN 
    TFlights AS TF ON TFP.intFlightID = TF.intFlightID
WHERE 
    TP.strFirstName = 'Ware' AND TP.strLastName = 'Hyjeked'
ORDER BY 
...
FROM 
    TPlanes AS TP
LEFT JOIN 
    TMaintenances AS TM ON TP.intPlaneID = TM.intPlaneID
WHERE 
    TM.intPlaneID IS NULL
-------6
SELECT 
    P.strFirstName AS strPilotFirstName,
    P.strLastName AS strPilotLastName,
    COUNT(PF.intPilotFlightID) AS intTotalFlights
FROM TPilots AS P
...
SELECT 
    ...
    PL.strPlaneNumber,
    PL.intPlaneTypeID,
    SUM(MMW.intHours) AS intTotalHours
FROM TPlanes AS PL
JOIN TMaintenances AS M ON PL.intPlaneID = M.intPlaneID
JOIN TMaintenanceMaintenanceWorkers AS MMW ON M.intMaintenanceID = MMW.intMaintenanceID
GROUP BY PL.intPlaneID, PL.strPlaneNumber, PL.intPlaneTypeID
-------8
SELECT 
    TP.strFirstName, 
    TP.strLastName, 
...
FROM 
    TPassengers AS TP
JOIN 
    TFlightPassengers AS TPF ON TP.intPassengerID = TPF.intPassengerID
JOIN 
    TFlights AS TF ON TPF.intFlightID = TF.intFlightID
GROUP BY 
...
HAVING 
    SUM(TF.intMilesFlown) = (
        ...
        FROM (
            SELECT 
                SUM(f.intMilesFlown) AS TotalMiles
            FROM 
                TPassengers AS TP
            JOIN 
                TFlightPassengers pf ON TP.intPassengerID = pf.intPassengerID
            JOIN 
                TFlights f ON pf.intFlightID = f.intFlightID
            GROUP BY 
                TP.intPassengerID
        ) AS MilesSub
    )
...
-------10
SELECT 
    strFirstName AS strPilotFirstName,
    strLastName AS strPilotLastName,
    AVG(intMilesFlown) AS decAverageMiles
FROM TPilots
JOIN TPilotFlights ON TPilots.intPilotID = TPilotFlights.intPilotID
JOIN TFlights ON TPilotFlights.intFlightID = TFlights.intFlightID
WHERE dtmFlightDate >= '2022-04-01' AND dtmFlightDate < '2022-05-01'
...
WHERE 
    NOT EXISTS (
        SELECT 1
        FROM 
            TFlightPassengers pf
        JOIN 
            TFlights f ON pf.FlightID = f.FlightID
        JOIN 
            TPilots pi ON f.intPilotID = pi.intPilotID
        JOIN 
            TFlightAttendants fa ON f.FlightID = fa.FlightID
        JOIN 
            TAttendants a ON fa.AttendantID = a.AttendantID
        WHERE 
            p.PassengerID = pf.PassengerID
            AND pi.strFirstName = 'Ima' AND pi.strLastName = 'Soring'
            AND a.strFirstName = 'Sherley' AND a.strLastName = 'Ujest'
    )
------15
SELECT 
    COUNT(*) AS intFlightCount
FROM TPilots AS P
JOIN TPilotFlights AS PF ON P.intPilotID = PF.intPilotID
JOIN TFlights AS F ON PF.intFlightID = F.intFlightID
JOIN TPlanes AS PL ON F.intPlaneID = PL.intPlaneID
JOIN TPlaneTypes AS PT ON PL.intPlaneTypeID = PT.intPlaneTypeID
WHERE 
    P.strFirstName = 'Tip' AND P.strLastName = 'Seenow'
    AND PT.strPlaneType = 'Boeing 747-8'
-----16
SELECT 
    strFirstName AS strPassengerFirstName,
    strLastName AS strPassengerLastName
FROM TPassengers
WHERE strAddress LIKE '%Oak%'
